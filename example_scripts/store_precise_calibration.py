...
configured = chess_config.populate_lens_parameters_from_chessboard(chessboard_image_path, chess_grid_rows, chess_grid_cols)

if not configured:
    print('Could not determine lens correction properties from {}'.format(chessboard_image_path))
    exit()
...
undistorted_dot_image = calib.correct_image(dot_grid_image, chess_config, calib.Correction.lens_distortion)
...
transformed_points = calib.correct_points(distorted_points, chess_config, calib.Correction.lens_distortion)
...
for i in range(sparse_rows):
    for j in range(sparse_cols):
        sparse_grid[i * sparse_cols + j, 0, 0] = distorted_grid[(i*2)*dot_grid_cols + (j*2), 0, 0]
        sparse_grid[i * sparse_cols + j, 0, 1] = distorted_grid[(i*2)*dot_grid_cols + (j*2), 0, 1]

# Generate the calibration configuration using the newly determined grids
h, w = dot_grid_image.shape[:2]
dot_config = calib.Config()
configured = dot_config.populate_lens_parameters_from_grid(sparse_grid, sparse_cols, sparse_rows, w, h)

if not configured:
    print('Could not determine distortion properties from {}'.format(dot_grid_image_path))
    exit()

undistorted_distorted_grid = calib.correct_points(distorted_grid, dot_config, calib.Correction.lens_distortion)
configured = dot_config.populate_keystone_and_real_parameters_from_grid(undistorted_distorted_grid,
                                                                        dot_grid_cols,
                                                                        dot_grid_rows,
                                                                        dot_grid_width, dot_grid_height,
                                                                        corners_only=corner_only_homography)

if not configured:
    print('Could not determine homography properties from {}'.format(dot_grid_image_path))
    exit()
